...
// Read the 8-bit value pointed to by the PC, setting inc_pc
static uint8_t next_byte(const Processor *proc, uint8_t *inc_pc) {
    uint8_t data = address_read(proc->c, proc->pc);
    if(inc_pc != NULL) *inc_pc = 1;
    return data;
}

// Read the 16-bit address pointed to by the PC, updating inc
static uint16_t next_address(const Processor *proc, uint8_t *inc_pc) {
    uint16_t address = address_read(proc->c, proc->pc);
    address |= address_read(proc->c, proc->pc + 1) << 8;
    if(inc_pc != NULL) *inc_pc = 2;
    return address;
}
...
uint16_t get_address_static(const Processor *proc, uint8_t *inc_pc) {
    ...
    switch(proc->inst.mode) {
        ...
        case MODE_INDIRECT_X:
            ...
            ptr = (next_byte(proc, inc_pc) + proc->x) & 0xFF;
            addr = address_read(proc->c, ptr);
...
uint8_t get_data_static(const Processor *proc, uint16_t *address,
    ...
    switch(proc->inst.mode) {
        ...
        default:
            ...
            addr = get_address_static(proc, inc_pc);
...
uint16_t get_address(Processor *proc) {
    uint8_t inc_pc;
    uint16_t address = get_address_static(proc, &inc_pc);
    proc->pc += inc_pc;
    return address;
}
...
uint8_t get_data(Processor *proc, uint16_t *address) {
    uint8_t inc_pc;
    uint8_t data = get_data_static(proc, address, &inc_pc);
    proc->pc += inc_pc;
    return data;
}
